	...
	function setUndefinedFieldsToNull(items) {
		_.each(items, function (item) {
            _.each(_metadata.fields, function(field) {
            	if (item[field.field] instanceof Array) {
            		_.each(item[field.field], function(item) {
            			_.each(field.type, function(field) {
...
/*new MongoRepository('/person').find({name: 'steve martin'}, function(err, items) {
	//console.log(items[0].imageData);
});*/
/*
